...
const BlogDetails = () => {
  ...
  if (!blog)
    return <p className="text-center mt-10 text-xl font-medium">Blog not found or loading...</p>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 shadow-xl rounded-xl p-6 h-full">
          ...
          <p className="text-gray-500 text-sm mb-2">
            {blog.date} | <span className="text-indigo-600 font-medium">{blog.category}</span>
          </p>
          <p className="text-gray-700 text-lg leading-relaxed">{blog.longDescription}</p>
        </div>
        ...
        <div className="bg-white border border-gray-200 shadow-xl rounded-xl p-6 h-full flex flex-col justify-between">
              ...
              <ul className="space-y-3 mb-6 max-h-64 overflow-y-auto pr-1">
                {comments.map((c, idx) => (
                  ...
                  >
                    ...
                    />
                    <div>
                      <p className="text-sm font-semibold text-gray-800">{c.userEmail || "Anonymous"}</p>
                      <p>{c.text}</p>
                    </div>
                  </li>
                ))}
          ...
          <form onSubmit={handleCommentSubmit}>
            <label htmlFor="comment" className="block mb-2 font-semibold text-gray-700">
              Leave a Comment:
            </label>
            <textarea
              id="comment"
