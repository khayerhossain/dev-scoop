    ...
    if (result.isConfirmed) {
      try {
        ...
        if (user) {
          ...
          Swal.fire('Deleted!', 'Your blog has been removed from wishlist.', 'success');
  ...
  return (
    <div className="bg-gray-100 py-10 px-5 sm:px-8 md:px-12 lg:px-16 xl:px-24">
      <div className="text-center mb-10">
        <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-2"> My Wishlist</h2>
        <p className="text-gray-600 text-base sm:text-lg max-w-2xl mx-auto">
          Here are the blogs you've saved to revisit later. Dive in, explore, or remove items you no longer need.
        </p>
      </div>
      ...
      {wishlist.length === 0 ? (
        <div className="max-w-md mx-auto bg-white border border-gray-200 rounded-xl shadow-md p-6 lg:py-16 text-center">
          ...
          />
          <h3 className="text-xl font-semibold text-gray-800 mb-2">You didn’t save any blogs yet</h3>
          <p className="text-gray-600 text-sm">Start exploring and add blogs you love to your wishlist!</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {wishlist.map((blog) => (
            <div
              key={blog._id}
              className="max-w-md mx-auto bg-gradient-to-br from-white/90 to-gray-100 backdrop-blur-md border border-gray-200 rounded-xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300"
            >
              <img
              ...
              <div className="p-5">
                ...
                <p className="text-gray-600 text-sm mb-6 leading-relaxed">
                  {showFull[blog._id] ? blog.longDescription : blog.shortDescription}
                </p>
                ...
                <div className="flex justify-between gap-3 text-sm font-semibold">
                  ...
                  <button
                    className="flex-1 px-4 py-2 rounded-lg border border-emerald-500 text-emerald-600 bg-white/70 hover:bg-emerald-500 hover:text-white transition-all duration-200 ease-in-out shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 cursor-pointer"
                  >
                    Details
                  </button>
